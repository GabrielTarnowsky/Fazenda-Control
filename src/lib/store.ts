...
export const store = {
  ...
  deleteAnimal: (id: string) => {
    ...
    supabase.from('animals').delete().eq('id', id).then();
  ...
  updateEvent: (id: string, data: Partial<AnimalEvent>) => {
    ...
    // Persist to Supabase
    supabase.from('events').update(data).eq('id', id).then();
  },
  addEvent: (e: Omit<AnimalEvent, "id">) => {
    ...
    // Persist to Supabase
    supabase.from('events').insert([item]).then();
    return item;
  },
  ...
  addFinancial: (f: Omit<Financial, "id">) => {
    ...
    // Persist to Supabase
    supabase.from('financial').insert([item]).then();
    return item;
  },
  ...
  addInsemination: (ins: Omit<Insemination, "id">) => {
    ...
    // Persist to Supabase
    supabase.from('inseminations').insert([item]).then();
    return item;
  },
  updateInsemination: (id: string, data: Partial<Insemination>) => {
    ...
    save("bovi_inseminations", list);
    // Persist to Supabase
    supabase.from('inseminations').update(data).eq('id', id).then();
  },
